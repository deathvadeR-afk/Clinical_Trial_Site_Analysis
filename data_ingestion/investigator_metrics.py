...
class InvestigatorMetricsCalculator:
    ...
    def calculate_publication_counts(self, publications: List[Dict]) -> Dict[str, int]:
        ...
        for pub in publications:
            ...
            pub_year_str = pub.get("publication_date", "")[
                :4
    ...
    def calculate_investigator_metrics(self, investigator_id: int) -> Dict[str, Any]:
        ...
        try:
            ...
            publication_results = self.db_manager.query(
                "SELECT * FROM pubmed_publications WHERE investigator_id = ?",
                (investigator_id,),
            )
            ...
            logger.error(
                f"Error calculating metrics for investigator {investigator_id}: {e}"
            )
            return {}
    ...
    def update_investigator_record(self, investigator_id: int) -> bool:
        ...
        try:
            ...
            if not metrics:
                logger.warning(
                    f"No metrics calculated for investigator {investigator_id}"
                )
                return False
