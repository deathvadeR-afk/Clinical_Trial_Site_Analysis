...
def fix_enrollment_counts():
    ...
    for nct_id, enrollment_count in trial_enrollments.items():
        ...
        cursor.execute(
            "SELECT COUNT(*) FROM clinical_trials WHERE nct_id = ?", (nct_id,)
        )
        if cursor.fetchone()[0] > 0:
            # Update the enrollment count
            cursor.execute(
                "UPDATE clinical_trials SET enrollment_count = ? WHERE nct_id = ?",
                (enrollment_count, nct_id),
            )
    ...
    for nct_id in trial_enrollments.keys():
        cursor.execute(
            "SELECT nct_id, enrollment_count FROM clinical_trials WHERE nct_id = ?",
            (nct_id,),
        )
...
def fix_date_formats():
    ...
    cursor.execute(
        "SELECT nct_id, start_date, completion_date FROM clinical_trials WHERE enrollment_count IS NOT NULL"
    )
    ...
    for nct_id, start_date, completion_date in trials:
        ...
        if start_date and len(start_date) == 7:  # Format like "2016-01"
            ...
            cursor.execute(
                "UPDATE clinical_trials SET start_date = ? WHERE nct_id = ?",
                (fixed_start, nct_id),
            )
        ...
        if completion_date and len(completion_date) == 7:  # Format like "2017-03"
            ...
            cursor.execute(
                "UPDATE clinical_trials SET completion_date = ? WHERE nct_id = ?",
                (fixed_completion, nct_id),
            )
