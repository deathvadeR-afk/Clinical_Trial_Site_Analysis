...
def analyze_database_for_ml():
    ...
    if complete_trials >= 100:
        ...
        print("❌ Insufficient data for predictive model training (need 50+ complete trials)")
    ...
    for row in intervention_data:
        if row['interventions']:
            # Parse JSON array
            try:
                interventions = json.loads(row['interventions'])  # Simple eval for JSON list
    ...
    quality_fields = ['has_nct_id', 'has_title', 'has_start_date', 'has_completion_date', 'has_phase', 'has_status', 'has_enrollment']
    for key in quality_fields:
        percentage = (trial_quality[key] / trial_quality['total']) * 100 if trial_quality['total'] > 0 else 0
        field_name = key.replace('has_', '').replace('_', ' ').title()
        print(f"  {field_name}: {trial_quality[key]:,}/{trial_quality['total']:,} ({percentage:.1f}%)")
    ...
    for key in ['has_name', 'has_city', 'has_country', 'has_lat', 'has_lon']:
        percentage = (site_quality[key] / site_quality['total']) * 100 if site_quality['total'] > 0 else 0
        field_name = key.replace('has_', '').replace('_', ' ').title()
        print(f"  {field_name}: {site_quality[key]:,}/{site_quality['total']:,} ({percentage:.1f}%)")
    ...
    if complete_trials >= 100:
        ml_readiness_score += 25
        ml_requirements.append("✅ Sufficient complete trials for predictive modeling (100+)")
    elif complete_trials >= 50:
        ml_readiness_score += 15
        ml_requirements.append("⚠️  Adequate complete trials for predictive modeling (50+)")
    else:
        ml_requirements.append("❌ Insufficient complete trials for predictive modeling (need 50+)")
