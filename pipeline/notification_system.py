    ...
    def send_email_notification(
        ...
        try:
            ...
            msg["From"] = self.email_config.get(
                "sender", "no-reply@clinical-trials-platform.com"
            )
...
"""
            ...
            if isinstance(issue_details, dict) and "count" in issue_details:
                ...
                if "details" in issue_details and issue_details["details"]:
                    ...
                    if len(issue_details["details"]) > 5:
                        body += (
                            f"    ... and {len(issue_details['details']) - 5} more\n"
                        )
            else:
    ...
    def send_system_alert(
        ...
        subject = f"{severity_emoji.get(severity, '⚠️')} System Alert: {alert_type} - {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}"
...
def main():
    ...
    success = notification_system.send_pipeline_completion_notification(
        success=True, duration=125.5, records_processed=42, errors=[]
    )
    ...
    issues = {
        "duplicate_sites": {"count": 3, "details": ["Site A", "Site B", "Site C"]},
