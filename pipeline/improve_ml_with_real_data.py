...
def download_real_historical_data():
    ...
    try:
        ...
        print(
            "Downloading real historical clinical trials data from ClinicalTrials.gov API..."
        )
        ...
        if predictive_model.is_configured:
            ...
            if df is not None and len(df) > 0:
                ...
                if training_success:
                    ...
                    performance_metrics = predictive_model.evaluate_model_performance(
                        df
                    )
    ...
    try:
        ...
        if clustering_model.is_configured:
            ...
            if clustering_results:
                clusters_found = len(
                    clustering_results.get("cluster_characteristics", {})
                )
                print(
                    f"SUCCESS: Site clustering completed with {clusters_found} clusters"
                )

                # Display cluster characteristics
                cluster_characteristics = clustering_results.get(
                ...
                if cluster_characteristics:
                    ...
                    for cluster_id, characteristics in cluster_characteristics.items():
                        ...
                        print(
                            f"    - Average completion ratio: {characteristics.get('average_completion_ratio', 0):.2f}"
                        )
                        print(
                            f"    - Average recruitment efficiency: {characteristics.get('average_recruitment_efficiency', 0):.2f}"
                        )
...
def verify_model_performance():
    ...
    try:
        ...
        # Check data counts
        site_count = db_manager.query("SELECT COUNT(*) as count FROM sites_master")[0][
            "count"
        ]
        trial_count = db_manager.query("SELECT COUNT(*) as count FROM clinical_trials")[
            0
        ]["count"]
        participation_count = db_manager.query(
            "SELECT COUNT(*) as count FROM site_trial_participation"
        )[0]["count"]
        metrics_count = db_manager.query("SELECT COUNT(*) as count FROM site_metrics")[
            0
        ]["count"]
        cluster_count = db_manager.query("SELECT COUNT(*) as count FROM site_clusters")[
            0
        ]["count"]
...
def main():
    ...
    print(
        f"Predictive Model Retraining: {'SUCCESS' if predictive_success else 'FAILED'}"
    )
    print(
        f"Clustering Model Retraining: {'SUCCESS' if clustering_success else 'FAILED'}"
    )
    print(
        f"Performance Verification: {'SUCCESS' if verification_success else 'FAILED'}"
    )

    overall_success = all(
        [data_success, predictive_success, clustering_success, verification_success]
    )
