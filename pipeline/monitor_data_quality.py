...
class DataQualityMonitor:
    ...
    def check_completeness(self) -> Dict[str, Any]:
        ...
        try:
            ...
            sites_result = self.db_manager.query("SELECT COUNT(*) as count FROM sites_master")
            sites_count = sites_result[0]['count'] if sites_result else 0
            
            sites_name_result = self.db_manager.query("SELECT COUNT(*) as count FROM sites_master WHERE site_name IS NOT NULL")
            ...
            trials_result = self.db_manager.query("SELECT COUNT(*) as count FROM clinical_trials")
            trials_count = trials_result[0]['count'] if trials_result else 0
            
            trials_title_result = self.db_manager.query("SELECT COUNT(*) as count FROM clinical_trials WHERE title IS NOT NULL")
            trials_with_title = trials_title_result[0]['count'] if trials_title_result else 0
            
            trials_completeness = trials_with_title / trials_count if trials_count > 0 else 0
            ...
            investigators_result = self.db_manager.query("SELECT COUNT(*) as count FROM investigators")
            investigators_count = investigators_result[0]['count'] if investigators_result else 0
            
            investigators_name_result = self.db_manager.query("SELECT COUNT(*) as count FROM investigators WHERE full_name IS NOT NULL")
            investigators_with_name = investigators_name_result[0]['count'] if investigators_name_result else 0
            
            investigators_completeness = investigators_with_name / investigators_count if investigators_count > 0 else 0
    ...
    def check_recency(self) -> Dict[str, Any]:
        ...
        try:
            ...
            # Check sites_master table
            sites_with_dates_result = self.db_manager.query("SELECT last_updated FROM sites_master WHERE last_updated IS NOT NULL")
            if sites_with_dates_result:
                ...
                recency_report['sites_master'] = {
                    ...
                    'recency_ratio': recent_sites / total_sites if total_sites > 0 else 0
                }
            
            # Check clinical_trials table
            trials_with_dates_result = self.db_manager.query("SELECT last_update_posted FROM clinical_trials WHERE last_update_posted IS NOT NULL")
            if trials_with_dates_result:
                ...
                recency_report['clinical_trials'] = {
                    ...
                    'recency_ratio': recent_trials / total_trials if total_trials > 0 else 0
                }
    ...
    def check_consistency(self) -> Dict[str, Any]:
        ...
        try:
            ...
            consistency_report['duplicate_sites'] = {
                ...
                'details': [{'site_name': row['site_name'], 'occurrences': row['count']} for row in duplicate_sites]
            }
            ...
            consistency_report['invalid_date_sequences'] = {
                ...
                'details': [{'nct_id': row['nct_id'], 'start_date': row['start_date'], 'completion_date': row['completion_date']} for row in invalid_dates]
            }
    ...
    def run_monitoring(self) -> bool:
        ...
        try:
            ...
            for table, metrics in report['completeness'].items():
                print(f"  {table}: {metrics['completeness_ratio']:.2%} ({metrics['complete_records']}/{metrics['total_records']})")
            
            # Recency summary
            print("\nRecency:")
            for table, metrics in report['recency'].items():
                print(f"  {table}: {metrics['recency_ratio']:.2%} ({metrics['recent_records']}/{metrics['total_with_dates']})")
            
            # Consistency summary
            print("\nConsistency Issues:")
            print(f"  Duplicate sites: {report['consistency']['duplicate_sites']['count']}")
            print(f"  Invalid date sequences: {report['consistency']['invalid_date_sequences']['count']}")
