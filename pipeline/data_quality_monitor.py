...
class DataQualityMonitor:
    ...
    def get_database_statistics(self) -> Dict:
        ...
        try:
            ...
            # Get table counts
            tables = ['sites_master', 'clinical_trials', 'site_trial_participation', 
                     'investigators', 'pubmed_publications', 'site_metrics', 
                     'data_quality_scores', 'match_scores']
            
            for table in tables:
                try:
                    result = self.db_manager.query(f"SELECT COUNT(*) as count FROM {table}")
                    if result:
    ...
    def save_report_to_file(self, report: Dict, output_path: Optional[str] = None) -> bool:
        """
        ...
        try:
            if output_path is None:
                ...
                output_path = os.path.join(reports_dir, f"data_quality_report_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json")
    ...
    def run_monitoring(self) -> bool:
        ...
        try:
            ...
            logger.info(f"  - Data Quality Score: {quality_metrics.get('overall_quality_score', 0):.2f}")
