...
class RecommendationEngine:
    ...
    def apply_mandatory_filtering_criteria(self, target_study: Dict[str, Any]) -> List[int]:
        """
    ...
    def execute_match_score_calculation(self, eligible_sites: List[int], 
                                     target_study: Dict[str, Any]) -> List[Dict[str, Any]]:
        """
        ...
        try:
            ...
            for site_id in eligible_sites:
                ...
                base_scores = self.match_calculator.calculate_match_scores_for_site(site_id, target_study)
                
                if base_scores:
                    # Apply experience-based adjustments
                    adjusted_scores = self.match_calculator.apply_experience_based_adjustments(
                        site_id, base_scores, target_study)
                    
                    # Store match scores
                    self.match_calculator.store_match_scores(site_id, target_study, adjusted_scores)
            ...
            site_scores.sort(key=lambda x: x['scores'].get('overall_match_score', 0), reverse=True)
    ...
    def implement_portfolio_optimization(self, site_scores: List[Dict[str, Any]], 
                                      target_study: Dict[str, Any]) -> List[Dict[str, Any]]:
        """
        ...
        try:
            ...
            for site_data in site_scores:
                ...
                if site_result:
                    country = site_result[0]['country'] if site_result[0]['country'] else 'Unknown'
            ...
            logger.info(f"Optimized site selection to {len(optimized_sites)} sites with geographic diversity")
    ...
    def generate_site_selection_tiers(self, optimized_sites: List[Dict[str, Any]]) -> Dict[str, List[Dict[str, Any]]]:
        """
        ...
        try:
            ...
            logger.info(f"Generated site selection tiers: "
    ...
    def create_recommendation_reports(self, tiers: Dict[str, List[Dict[str, Any]]], 
                                   target_study: Dict[str, Any]) -> Dict[str, Any]:
        """
            ...
            for tier_name, sites in tiers.items():
                ...
                for site_data in sites:
                    ...
                    if site_result:
                        ...
                        strengths_weaknesses = self.strengths_detector.generate_structured_strengths_weaknesses(site_id)
    ...
    def support_alternative_scenarios(self, base_target_study: Dict[str, Any]) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            scenarios['conservative']['phase'] = 'Phase 3'  # Typically higher completion
            ...
            for scenario_name, scenario_params in scenarios.items():
                eligible_sites = self.apply_mandatory_filtering_criteria(scenario_params)
                site_scores = self.execute_match_score_calculation(eligible_sites, scenario_params)
                optimized_sites = self.implement_portfolio_optimization(site_scores, scenario_params)
    ...
    def enable_interactive_refinement(self, current_recommendations: Dict[str, Any], 
                                   refinement_params: Dict[str, Any]) -> Dict[str, Any]:
        """
            ...
            if 'preferred_countries' in refinement_params:
                ...
                for tier_name, sites in refined_recommendations['tiers'].items():
                    filtered_sites = [
                        ...
                        if site.get('site_info', {}).get('country') in preferred_countries
                    ]
    ...
    def generate_recommendations(self, target_study: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            site_scores = self.execute_match_score_calculation(eligible_sites, target_study)
            if not site_scores:
                logger.warning("No match scores calculated")
                return {}
            
            # Optimize portfolio
            optimized_sites = self.implement_portfolio_optimization(site_scores, target_study)
