...
class MetricsCalculator:
    ...
    def aggregate_trial_participation_data(self, site_id: int) -> Dict[str, Any]:
        ...
        try:
            ...
            # Calculate averages
            avg_enrollment = total_enrollment / total_studies if total_studies > 0 else 0
            avg_enrollment_duration = sum(enrollment_durations) / len(enrollment_durations) if enrollment_durations else 0
            
            # Calculate completion ratio
            completion_ratio = completed_studies / total_studies if total_studies > 0 else 0
            ...
            logger.error(f"Error aggregating trial participation data for site {site_id}: {e}")
            return {}
    ...
    def build_therapeutic_area_taxonomy(self) -> Dict[str, List[str]]:
        ...
        try:
            ...
            for row in condition_results:
                ...
                try:
                    ...
                    for condition in conditions:
                        ...
                        if any(keyword in condition_lower for keyword in ['cancer', 'tumor', 'carcinoma', 'sarcoma']):
                            therapeutic_areas['Oncology'].append(condition)
                        elif any(keyword in condition_lower for keyword in ['diabetes', 'glucose', 'insulin']):
                            therapeutic_areas['Endocrinology'].append(condition)
                        elif any(keyword in condition_lower for keyword in ['heart', 'cardiac', 'cardiovascular', 'hypertension']):
                            therapeutic_areas['Cardiology'].append(condition)
                        elif any(keyword in condition_lower for keyword in ['infect', 'virus', 'bacteria', 'viral', 'bacterial']):
                            therapeutic_areas['Infectious Disease'].append(condition)
                        elif any(keyword in condition_lower for keyword in ['mental', 'depression', 'anxiety', 'psych']):
                            therapeutic_areas['Psychiatry'].append(condition)
                        elif any(keyword in condition_lower for keyword in ['neuro', 'brain', 'parkinson', 'alzheimer']):
            ...
            logger.info(f"Built therapeutic area taxonomy with {len(therapeutic_areas)} areas")
    ...
    def calculate_temporal_metrics(self, site_id: int) -> Dict[str, Any]:
        ...
        try:
            ...
            yearly_metrics: Dict[int, Dict[str, Any]] = defaultdict(lambda: {
                'studies_count': 0,
                'total_enrollment': 0,
                'phase_distribution': {}
            })
            ...
            for year, metrics in yearly_metrics.items():
                metrics['avg_enrollment_per_study'] = (
                    ...
                    if metrics['studies_count'] > 0 else 0
                )
                temporal_metrics[year] = dict(metrics)
                # Convert phase_distribution to regular dict
                temporal_metrics[year]['phase_distribution'] = dict(metrics['phase_distribution'])
    ...
    def aggregate_investigator_data(self, site_id: int) -> Dict[str, Any]:
        ...
        try:
            ...
            # Calculate averages
            avg_h_index = total_h_index / total_investigators if total_investigators > 0 else 0
            avg_publications = total_publications / total_investigators if total_investigators > 0 else 0
            avg_recent_publications = total_recent_publications / total_investigators if total_investigators > 0 else 0
    ...
    def populate_sites_master_table(self) -> bool:
        ...
        try:
            ...
            for row in site_results:
                ...
                if trial_data:
                    update_data['total_capacity'] = trial_data.get('total_enrollment', 0)
                ...
                if update_data:
                    ...
                    if self.db_manager.execute(sql, tuple(values)):
                        updated_count += 1
                        logger.info(f"Updated site {site_id} with aggregated attributes")
            
            logger.info(f"Populated sites_master table with aggregated attributes for {updated_count} sites")
            return True
