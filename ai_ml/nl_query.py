...
class NLQueryProcessor:
    ...
    def execute_generated_queries(
        ...
        try:
            ...
            if query_type == "site_search":
                sql = self._generate_site_search_query(
                    therapeutic_areas, phases, numerical_values
                )
            elif query_type == "trial_search":
                sql = self._generate_trial_search_query(
                    therapeutic_areas, phases, numerical_values
                )
            elif query_type == "investigator_search":
                sql = self._generate_investigator_search_query(
                    therapeutic_areas, numerical_values
                )
                ...
                sql = self._generate_recommendation_query(
                    therapeutic_areas, phases, numerical_values
                )
            else:
            ...
            logger.info(
                f"Executed query and retrieved {len(formatted_results)} results"
            )
    ...
    def _generate_site_search_query(
        self,
        ...
        if therapeutic_areas:
            area_conditions = " OR ".join(
                [f"si.therapeutic_area LIKE '%{area}%'" for area in therapeutic_areas]
            )
    ...
    def _generate_trial_search_query(
        self,
        therapeutic_areas: List[str],
        phases: List[str],
        numerical_values: List[float],
        ...
        if therapeutic_areas:
            area_conditions = " OR ".join(
                [f"conditions LIKE '%{area}%'" for area in therapeutic_areas]
            )
            conditions.append(f"({area_conditions})")

        if phases:
            phase_conditions = " OR ".join(
                [f"phase LIKE '%{phase}%'" for phase in phases]
            )
    ...
    def _generate_investigator_search_query(
        self, therapeutic_areas: List[str], numerical_values: List[float]
    ) -> str:
    ...
    def _generate_performance_analysis_query(
    ...
    def generate_natural_language_responses(
        ...
        try:
            ...
            if "recommend" in query.lower():
                response = (
                    f"I found {result_count} recommended sites based on your query. "
                )
                if results:
                    top_sites = results[:3]
                    site_names = [
                        site.get("site_name", "Unknown Site") for site in top_sites
                    ]
                    response += f"The top recommendations are: {', '.join(site_names)}."
            elif "performance" in query.lower() or "ratio" in query.lower():
                response = (
                    f"I found {result_count} sites matching your performance criteria. "
                )
                if results:
                    avg_completion = sum(
                        site.get("completion_ratio", 0) for site in results
                    ) / len(results)
                    response += f"The average completion ratio is {avg_completion:.2f}."
            elif "investigator" in query.lower():
                response = (
                    f"I found {result_count} investigators matching your criteria. "
                )
                if results:
                    avg_h_index = sum(site.get("h_index", 0) for site in results) / len(
                        results
                    )
                    response += f"The average h-index is {avg_h_index:.1f}."
            else:
                response = (
                    f"I found {result_count} results matching your query: '{query}'. "
                )
                if results:
