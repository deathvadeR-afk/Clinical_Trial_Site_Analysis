...
class SiteClustering:
    ...
    def __init__(self, db_manager):
        ...
        if not SKLEARN_AVAILABLE:
            logger.warning("Scikit-learn not installed. Clustering functionality will be limited.")
    ...
    def construct_textual_site_profiles(self) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            for row in site_results:
                ...
                if therapy_results:
                    profile['therapeutic_areas'] = [r['therapeutic_area'] for r in therapy_results]
                ...
                if investigator_results and investigator_results[0]:
                    profile['investigator_strength'] = investigator_results[0]['avg_h_index'] or 0
    ...
    def generate_embeddings(self, site_profiles: List[Dict[str, Any]]) -> Optional[np.ndarray]:
        """
    ...
    def implement_dimensionality_reduction(self, embeddings: np.ndarray) -> Optional[np.ndarray]:
        """
    ...
    def apply_clustering_algorithms(self, embeddings: np.ndarray, n_clusters: int = 5) -> Optional[List[int]]:
        """
    ...
    def characterize_each_cluster(self, site_profiles: List[Dict[str, Any]], 
        ...
        try:
            ...
            for cluster_id, sites in clusters.items():
                ...
                recruitment_efficiency = np.mean([s['recruitment_efficiency'] for s in sites])
                experience_index = np.mean([s['experience_index'] for s in sites])
                investigator_strength = np.mean([s['investigator_strength'] for s in sites])
                ...
                common_therapies = [therapy for therapy, count in therapy_counts.most_common(3)]
                ...
                common_institutions = [inst for inst, count in institution_counts.most_common(2)]
    ...
    def calculate_cluster_quality_metrics(self, embeddings: np.ndarray, 
    ...
    def use_clustering_insights_for_recommendations(self, site_profiles: List[Dict[str, Any]], 
                                                 cluster_labels: List[int]) -> List[Dict[str, Any]]:
        """
        ...
        try:
            ...
            for cluster_id, sites in clusters.items():
                ...
                sorted_sites = sorted(sites, key=lambda x: x['completion_ratio'], reverse=True)
            ...
            logger.info(f"Generated clustering-based recommendations for {len(recommendations)} clusters")
    ...
    def store_clustering_results(self, site_profiles: List[Dict[str, Any]], 
                              cluster_labels: List[int], 
                              cluster_characteristics: Dict[int, Dict[str, Any]]) -> bool:
        """
    ...
    def perform_site_clustering(self, n_clusters: int = 5) -> Dict[str, Any]:
        ...
        try:
            ...
            cluster_characteristics = self.characterize_each_cluster(site_profiles, cluster_labels)
            
            # Step 6: Calculate cluster quality metrics
            logger.info("Step 6: Calculating cluster quality metrics")
            quality_metrics = self.calculate_cluster_quality_metrics(embeddings, cluster_labels)
            
            # Step 7: Generate recommendations
            logger.info("Step 7: Generating clustering-based recommendations")
            recommendations = self.use_clustering_insights_for_recommendations(site_profiles, cluster_labels)
            
            # Step 8: Store results
            logger.info("Step 8: Storing clustering results")
            self.store_clustering_results(site_profiles, cluster_labels, cluster_characteristics)
