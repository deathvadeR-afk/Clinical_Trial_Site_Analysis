...
class PredictiveEnrollmentModel:
    ...
    def __init__(self, db_manager):
        ...
        if not SKLEARN_AVAILABLE:
            logger.warning("Scikit-learn not installed. Predictive modeling functionality will be limited.")
    ...
    def prepare_training_dataset(self) -> Optional[pd.DataFrame]:
        ...
        try:
            ...
            df['institution_type_encoded'] = pd.Categorical(df['institution_type']).codes
            
            # Calculate enrollment duration
            df['start_date'] = pd.to_datetime(df['start_date'])
            df['completion_date'] = pd.to_datetime(df['completion_date'])
            df['enrollment_duration_days'] = (df['completion_date'] - df['start_date']).dt.days
    ...
    def engineer_additional_features(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        try:
            ...
            df['enrollment_rate'] = df['enrollment_count'] / (df['enrollment_duration_days'] / 30)  # Monthly rate
            df['completion_ratio_squared'] = df['completion_ratio'] ** 2
            df['experience_interaction'] = df['experience_index'] * df['recruitment_efficiency_score']
            df['phase_country_interaction'] = df['phase_encoded'] * df['country_encoded']
    ...
    def evaluate_model_performance(self, df: pd.DataFrame) -> Dict[str, float]:
        ...
        try:
            ...
            logger.info(f"Model evaluation - RMSE: {rmse:.2f}, R²: {r2:.3f}, MAPE: {mape:.2f}%")
    ...
    def implement_prediction_intervals(self, df: pd.DataFrame, confidence_level: float = 0.95) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            logger.info(f"Implemented prediction intervals with {confidence_level*100}% confidence")
    ...
    def use_gemini_api_for_prediction_explanations(self, predictions: List[float], 
                                               site_data: List[Dict[str, Any]]) -> List[Optional[str]]:
        """
    ...
    def integrate_enrollment_predictions(self, target_study: Dict[str, Any]) -> List[Dict[str, Any]]:
        """
        ...
        try:
            ...
            for row in site_results:
                ...
                features = {
                    ...
                    'institution_type_encoded': hash(row['institution_type'] or '') % 100,
                    'completion_ratio': row['completion_ratio'] or 0,
                    'recruitment_efficiency_score': row['recruitment_efficiency_score'] or 0,
                    ...
                    'experience_interaction': (row['experience_index'] or 0) * (row['recruitment_efficiency_score'] or 0),
                    'phase_country_interaction': 2 * (hash(row['country'] or '') % 1000)  # Phase 2 * country
                }
            ...
            explanations = self.use_gemini_api_for_prediction_explanations(predictions.tolist(), site_data)
            
            # Create recommendations
            recommendations = []
            for i, (site, prediction, explanation) in enumerate(zip(site_data, predictions, explanations)):
                recommendation = {
