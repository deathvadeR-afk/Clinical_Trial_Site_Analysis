...
def test_match_calculator(db_manager):
    ...
    adjusted_scores = match_calculator.apply_experience_based_adjustments(1, scores, target_study)
    if adjusted_scores:
        print("✅ Experience-based adjustments test passed")
        print(f"   Adjusted overall match score: {adjusted_scores.get('overall_match_score', 0):.3f}")
    else:
...
def test_recommendation_engine(db_manager):
    ...
    print(f"✅ Generated tiers: Primary={len(tiers['primary'])}, Secondary={len(tiers['secondary'])}, Tertiary={len(tiers['tertiary'])}")
