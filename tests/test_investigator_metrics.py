    ...
    if db_manager.connect():
        ...
        if db_manager.create_tables("database/schema.sql"):
            ...
            sample_publications = [
                ...
                {"publication_date": "2020-03-10", "publication_type": "Review"},
                {
            ...
            pub_counts = metrics_calculator.calculate_publication_counts(
                sample_publications
            )
            ...
            focus_analysis = metrics_calculator.analyze_research_focus(
                sample_publications_with_keywords
            )
            print("✓ Research focus analysis completed:")
            print(f"  Primary focus areas: {focus_analysis['primary_focus_areas']}")
            print(
                f"  Collaboration patterns: {focus_analysis['collaboration_patterns']}"
            )
            ...
            if db_manager.insert_data("investigators", test_investigator):
                ...
                results = db_manager.query(
                    "SELECT investigator_id FROM investigators WHERE full_name = ?",
                    ("Dr. Jane Smith",),
                )
                if results:
                    ...
                    if db_manager.insert_many("pubmed_publications", test_publications):
                        ...
                        # Calculate metrics
                        metrics = metrics_calculator.calculate_investigator_metrics(
                            investigator_id
                        )
                        if metrics:
                            ...
                            print(
                                f"  Total publications: {metrics['total_publications']}"
                            )
                            print(
                                f"  Recent publications: {metrics['recent_publications']}"
                            )
                            print(
                                f"  Primary focus areas: {metrics['primary_focus_areas']}"
                            )

                            # Update investigator record
                            if metrics_calculator.update_investigator_record(
                                investigator_id
                            ):
