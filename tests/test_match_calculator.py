    ...
    if db_manager.connect():
        ...
        if db_manager.create_tables("database/schema.sql"):
            ...
            if db_manager.insert_data("sites_master", test_site):
                ...
                # Get the site ID
                results = db_manager.query("SELECT site_id FROM sites_master WHERE site_name = ?", 
                                         ("Test Medical Center",))
                if results:
                    ...
                    scores = match_calculator.calculate_match_scores_for_site(site_id, target_study)
                    if scores:
                        ...
                        print(f"  Intervention: {scores['intervention_match_score']:.3f}")
