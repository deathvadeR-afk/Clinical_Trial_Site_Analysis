...
class CacheManager:
    ...
    def __init__(self, cache_dir: str = "../cache", default_ttl: int = 3600):
        ...
        logger.info(
            f"CacheManager initialized with cache_dir={cache_dir}, default_ttl={default_ttl}"
        )
    ...
    def _get_cache_file_path(self, key: str) -> str:
        ...
        if len(safe_key) > max_key_length:
            ...
            safe_key = (
                hash_object.hexdigest()[:50] + "_" + safe_key[-100:]
                if len(safe_key) > 150
                else safe_key[:max_key_length]
            )
    ...
    def set(self, key: str, value: Any, ttl: Optional[int] = None) -> bool:
        ...
        try:
            ...
            logger.debug(
                f"Cache set for key: {key} (expires in {ttl or self.default_ttl} seconds)"
            )
            return True
